...
def main():
    ...
    train_job = command(
        inputs=dict(
            train_data=Input(type="uri_folder", path="azureml://datastores/workspaceblobstore/paths/train/"),
            test_data=Input(type="uri_folder", path="azureml://datastores/workspaceblobstore/paths/test/"),
            n_estimators=100,
            max_depth=-1,
        ),
        outputs=dict(
